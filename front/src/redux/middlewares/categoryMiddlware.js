...
import { addActivePropertyToAttributes } from "../../components/helper";
...
export const categoryMiddleware = (store) => (next) => (action) => {
  switch (action.type) {
    case INITIALIZE:
      let { payload } = action;
      let categoryName =
        payload.includes("all") ||
        payload.includes("tech") ||
        payload.includes("clothes")
          ? payload
          : "all";
      ...
      Promise.all([categoryCall, currencyCall, productsCall])
        .then((response) => {
          action.payload = {
            categories: response[0].categories.map((category, i) => {
              return {
                name: category.name,
                selected: category.name === categoryName,
              };
            }),
            currencies: response[1].currencies.map((x, i) => {
              return { label: x.label, symbol: x.symbol, selected: i === 0 };
            }),
            products: addActivePropertyToAttributes(
              response[2].category.products
            ),
          };
          next(action);
        })
