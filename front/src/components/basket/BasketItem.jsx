...
class BasketItem extends Component {
  ...
  getAmount = ({ prices }) => {
    const currentCurrency = this.props.currency.currencies.find(
      (x) => x.selected
    );
    const price = prices.find(
      (x) => x.currency.label === currentCurrency.label
    );
        ...
        <div className={cssClass}>
          ...
          <div className="item-amount">
            ...
            <div className="cart-img">
              <img src={product.gallery[counter]} alt="cart" />
            </div>
            {product.gallery.length > 1 ? (
              <div className="arrows">
                <span className="arrow" onClick={this.decreaseCounter}>
                  &#8249;
                </span>
                <span className="arrow" onClick={this.increaseCounter}>
                  &#8250;
                </span>
              </div>
            ) : (
